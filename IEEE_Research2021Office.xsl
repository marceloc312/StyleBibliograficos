...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template name="BibDisplayDOI">
    ...
    <xsl:if test ="$cDOI!=0">
      <xsl:choose>
        <xsl:when test="contains(b:DOI,'org/')">
          <xsl:call-template name="right-trim">
            <xsl:with-param name ="s" select="substring-after(b:DOI, 'org/')" />
          </xsl:call-template>
        </xsl:when>
        <xsl:otherwise>
          <xsl:call-template name="right-trim">
            ...
            <xsl:element name="p">
              ...
              <xsl:element name="a">
                <xsl:attribute name="href">
                  <xsl:call-template name ="BibDisplayLinkDOI"/>
                </xsl:attribute>
